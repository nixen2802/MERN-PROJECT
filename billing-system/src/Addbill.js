...
class Addbill extends Component {
	...
	handleInputChange = (i, e) => {
		...
		for(let i=0;i<this.state.products.length;i++)
		{
			...
			{
				console.log(this.state.products[i].name,this.state.billValues[j].product_name);
	...
	addFormFields() {
		this.setState(({
		  billValues: [...this.state.billValues, {product_name: "",
		  quantity: "",
		  amount: "",}]
		}))
	  }
	...
	handleSubmit = (e) => {
		...
		const { billnumber, customer_name, date_of_supply, place_of_supply, transporter_info, gst_no, billing_address, billValues } = this.state;

		const bill = {
			billnumber, customer_name, date_of_supply, place_of_supply, transporter_info, gst_no, billing_address, billValues
		};
		this.setState({
	...
	render() {
		...
		let customer_options = this.state.customers.map(v => (
			<option value={v.id} style={{color: "black"}}>{v.cust_name}</option>
		));
		return (
			<div>
				<div class="limiter">
					<div class="container-login100">
						...
						<form  onSubmit={this.handleSubmit}>
							...
							<select name="customer_name" value={this.state.customer_name} onChange={this.handleChange}>
								<option>Select</option>
									{customer_options}
							</select>
							<input type="date" name="date_of_supply" value={this.state.date_of_supply} onChange={this.handleChange} />
							<br />
							GST NO : <input type="text" name="gst_no" value={this.state.gst_no} onChange={this.handleChange} />
							<br />
							Billing Address : <input type="text" name="billing_address" value={this.state.billing_address} onChange={this.handleChange} />
							<br />
							<input
								class="input100"
								type="text"
							...
							{this.state.billValues.map((element, index) => (
								<div className="form-inline" key={index}>
									<select name="product_name" value={element.product_name || ""} onChange={e=>this.handleInputChange(index,e)}>
									<option>Select</option>
										{options}
									</select>
									<input
