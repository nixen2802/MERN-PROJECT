...
class Showbill extends React.Component{
    ...
    componentDidMount() {
        axios.get('http://localhost:5000/fetch_bills',{params: {id: this.state.billnumber}}).then((result)=>{
            console.log(result.data)
            this.setState({
                bills: result.data
              });
        })
        axios.get('http://localhost:5000/fetch_bill_details',{params: {id: this.state.billnumber}}).then((result)=>{
            this.setState({
